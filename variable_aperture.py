...
def variable_aperture(data,min_brightness,a,b):
    """
    ...
    while np.amax(data) > min_brightness:
        ...
        for i in range(len(l)):
            ...
            # Local background
            bkg = ap.annulus_background(data,l[i][0],l[i][1],r+a,r+b)
            
            # Total flux in aperture, corrected with local background
            c_flux = ap.corrected_flux(data,l[i][0],l[i][1],r,r+a,r+b)
            
            data = ap.data_after_mask(data,l[i][0],l[i][1],r)
            
            fin = time.time() - start_time
            
            print('\nGalaxy found at (%s,%s)! Corrected flux = %s. Local background = %s' % (l[i][1],l[i][0],c_flux,bkg))
            print("--- %s seconds to find ---" % fin)
...
def variable_aperture2(data,min_brightness,min_radius,a,b,cal,e_cal):
    ...
    np.savetxt('catalogue_14_3465_3_9.txt', storage, header="ID\tx\ty\tFlux\tFlux error\tBkg\tBkg error\tm\tm error",delimiter='\t',fmt='%0.4e')
...
def variable_aperture3(data,min_brightness,min_radius,a,b,cal,e_cal):
    """
    ...
    while np.amax(data) > min_brightness:
        ...
        for i in range(len(l)):
            ...
            if c_flux > 0 and max_value < 8000 and r >= min_radius:
                ...
                storage.append(np.array([count,l[i][1]+1,l[i][0]+1,max_value,r,c_flux,e_c,bkg,e_b,m,e_m]))
                    
    
    np.savetxt('catalogue_final_3465_3_9.txt', storage, header="ID\tx\ty\tVal\tRadius\tFlux\tFlux error\tBkg\tBkg error\tm\tm error",delimiter='\t',fmt='%0.4e')

    return data,count

#%%

def variable_aperture_clean(data,min_brightness,min_radius,a,b,cal,e_cal):
    """
    ...
    np.savetxt('catalogue_smaller_3465_3_9.txt', storage, header="ID\tx\ty\tFlux\tFlux error\tBkg\tBkg error\tm\tm error",delimiter='\t',fmt='%0.4e')
...
start_time = time.time()
data_complete,count = variable_aperture3(small_patch,3465,3,9,cal,cal_error)
...
plt.imshow(small_patch,interpolation='nearest',origin='lower',cmap='Greys_r')
plt.xlabel('x (pixels)')
plt.ylabel('y (pixels)')
cbar = plt.colorbar()
...
plt.imshow(data_complete,interpolation='nearest',origin='lower',cmap='Greys_r')
plt.xlabel('x (pixels)')
plt.ylabel('y (pixels)')
cbar = plt.colorbar()
cbar.set_label('Pixel value')
...
start_time = time.time()
data_complete,count = variable_aperture3(small_patch1,3465,3,3,9,cal,cal_error)
print('Number of galaxies found = %s' %count)
fin = time.time() - start_time
print("Total time = %s seconds ---" % fin)
...
plt.imshow(np.log(data_complete),interpolation='nearest',origin='lower')
plt.xlabel('x (pixels)')
plt.ylabel('y (pixels)')
cbar = plt.colorbar()
cbar.set_label('ln(pixel value)')
...
plt.imshow(np.log(real_data),interpolation='nearest',origin='lower')
plt.title('Before (log)')
plt.xlabel('x')
plt.ylabel('y')
plt.colorbar()
#plt.savefig('before-small-patch-log.png',bbox_inches='tight')

#%%

plt.imshow(tested_data,interpolation='nearest',origin='lower')
plt.title('After (log)')
plt.xlabel('x')
plt.ylabel('y')
plt.colorbar()
